...
def sentence_phonemes(kokoro: Kokoro, text: str, pattern):
    segments = language_segments(text, pattern)
...
def synthesize(kokoro: Kokoro, style, speed: float, text: str, pattern):
    chunks = [np.zeros(int(24_000 * 0.30), dtype="float32")]
    sample_rate = 24_000
    sentences = split_sentences(text)
    audit = []
    for index, sentence in enumerate(sentences):
        phonemes, segments = sentence_phonemes(kokoro, sentence, pattern)
...
def main():
    ...
    vocabulary = [] if isinstance(payload, list) else payload.get("spanishTerms", [])
    pattern = spanish_pattern(vocabulary)
    args.output_dir.mkdir(parents=True, exist_ok=True)
    audit = {"profile": args.profile, "utterances": []}
    for utterance in utterances:
        audio, sample_rate, sentences = synthesize(kokoro, style, profile["speed"], utterance["text"], pattern)
