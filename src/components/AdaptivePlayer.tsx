...
export function AdaptivePlayer({ section }: { section: Section }) {
  ...
  function playCurrent() {
    ...
    const narration = new Audio(`/media/${section.id}/slides/${String(slideIndex).padStart(2, "0")}.mp3`);
    audio.current?.pause();
  ...
  return <section className="adaptive-player" aria-labelledby="instruction-title">
    <header className="player-header"><div><p className="eyebrow">Instruction</p><h2 id="instruction-title">Adaptive lesson presentation</h2></div>
      <div className="mode-tabs" role="tablist" aria-label="Instruction format">
        {(["adaptive","video","transcript"] as const).map((item) => <button key={item} type="button" role="tab" aria-selected={mode === item} onClick={() => { stop(); setMode(item); if (item === "video") void cacheForOffline([section.media.video, section.media.audio]); }}>{item === "adaptive" ? "Adaptive slides" : item === "video" ? "MP4 video" : "Transcript"}</button>)}
      </div>
    </header>
