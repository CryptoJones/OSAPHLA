...
export function AdaptivePlayer({ section }: { section: Section }) {
  ...
  return <section className="adaptive-player" aria-labelledby="instruction-title">
    <header className="player-header"><div><p className="eyebrow">{spanish ? "Instrucción" : "Instruction"}</p><h2 id="instruction-title">{spanish ? "Presentación adaptable de la lección" : "Adaptive lesson presentation"}</h2></div>
      <div className="mode-tabs" role="tablist" aria-label={spanish ? "Formato de instrucción" : "Instruction format"}>
        {(["adaptive","video","transcript"] as const).map((item) => <button key={item} type="button" role="tab" aria-selected={mode === item} onClick={() => { stop(); setMode(item); if (item === "video") void cacheForOffline([mediaUrl(section.media.video), mediaUrl(section.media.audio)]); }}>{item === "adaptive" ? (spanish ? "Diapositivas" : "Adaptive slides") : item === "video" ? (spanish ? "Video MP4" : "MP4 video") : (spanish ? "Transcripción" : "Transcript")}</button>)}
      </div>
    </header>
    {mode === "adaptive" && <div className="slide-stage" role="group" aria-roledescription={spanish ? "diapositiva narrada adaptable" : "themeable narrated slide"} aria-label={spanish ? `Diapositiva ${slideIndex + 1} de ${section.slides.length}: ${slide.title}` : `Slide ${slideIndex + 1} of ${section.slides.length}: ${slide.title}`}>
    ...
    {mode === "transcript" && <article className="transcript"><h3>{spanish ? "Transcripción descriptiva" : "Descriptive transcript"}</h3>{section.media.transcript.split("\n\n").map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</article>}
  </section>;
}
